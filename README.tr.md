...
| [Volume Profile Plus](indicators/02-volume-profile-plus/) | Seans bazlı VP, POC, VAH/VAL, Hacim Düğümleri | ✅ Canlı |
| [MTF Divergence Scanner](indicators/03-mtf-divergence/) | Çoklu zaman dilimi RSI/MACD ıraksaklık tespiti | ✅ Canlı |
| [ATR Helper](indicators/04-atr-helper/) | Dinamik SL/TP, R:R görselleştirmesi, pozisyon büyüklüğü | ✅ Canlı |

### 💎 Pro (Davetiyeli Erişim)

| Gösterge | Amaç | Durum |
|----------|------|-------|
| [Pro Confluence Engine](pro/) | 4 ücretsiz gösterge konseptini adaptif skorlama ile birleştiren tek elden sinyal sistemi | ✅ Hazır — davetiyeli |

> Pro Confluence Engine'in kaynak kodu bu repoda **bulunmaz** (bkz. [LICENSE-PRO](LICENSE-PRO)). Yalnızca TradingView'in davetiyeli (invite-only) sistemi üzerinden dağıtılır; [pro/](pro/) klasörü sadece dokümantasyon içerir.
...
- **`request.security()`** repaint etmeyen onaylı-bar deseniyle çoklu zaman dilimi verisi için (MTF Divergence Scanner, Pro Confluence Engine)
- **Kullanıcı tanımlı tipler** temiz state yönetimi için
- **Metodlar (dot notation)** okunabilir array/matrix işlemleri için
- **Onaylı-bar kapılama** (`barstate.isconfirmed`) repaint-güvenli sinyal ve alarmlar için
- **Strict typing** baştan sona
...
- [x] SMC Toolkit v1.1.0 — Order Blocks, FVG, Likidite Sweep'leri, BoS/CHoCH ([detaylar](indicators/01-smc-toolkit/))
- [x] Volume Profile Plus (POC/VAH/VAL ile seans bazlı)
- [x] MTF Divergence Scanner (Zaman dilimleri arasında RSI/MACD)
- [x] ATR Helper (Dinamik SL/TP ve R:R görselleştirmesi)
- [x] Pro Confluence Engine v1.0.0 — adaptif konfluans skorlaması; davetiyeli, kaynak repoda değil ([dokümanlar](pro/))
